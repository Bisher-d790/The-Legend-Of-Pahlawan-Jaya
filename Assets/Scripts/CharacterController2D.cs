...
public class CharacterController2D : MonoBehaviour {
	...
	public Transform groundCheck_Center;
	...
	void Update()
	{
		...
		// whatIsGround layer
        _isGrounded = (Physics2D.Linecast(_transform.position, groundCheck_Center.position, whatIsGround) || Physics2D.Linecast(_transform.position, groundCheck_Right.position, whatIsGround) || Physics2D.Linecast(_transform.position, groundCheck_Left
		...
		else if(_canDoubleJump && CrossPlatformInputManager.GetButtonDown("Jump"))//If grounded and can double jump and jump button pressed then double jump
		{
				doJump();
				_canDoubleJump = false; // can double jump once while in air
		}
    ...
    public void Checkpoint (Vector3 position)
    {
        ...
        if (GameManager.gm) // add the points through the game manager, if it is available
            GameManager.gm._spawnLocation = position;
    }
